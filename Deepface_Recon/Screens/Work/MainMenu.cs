...
public partial class MainMenu : Form
{
    ...
    private void Iniciar_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (ApiFlag == 0)
            {
                var APIThread = new Thread(APIControl);
                APIThread.Start();
    ...
    private static void APIControl()
    {
        APIEngine.ApiRun();
    }
    ...
    {
        var Explica = new Thread(CallExplicação);
        Explica.Start();
    }
    
    private void CallExplicação()
    {
        Application.Run(new Explica());
    }
    ...
    {
        Process[] AguiaEyeInstances = Process.GetProcessesByName("Aguia Eye");
        if(AguiaEyeInstances.Length > 0)
        {
            foreach (Process AguiaEyeInstance in AguiaEyeInstances)
            {
                AguiaEyeInstance.Kill();
            }
        }
    }
}